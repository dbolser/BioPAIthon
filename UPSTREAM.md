...
| 18 | `neighbor_simple_search` re-sorts the KD-tree's shared point list | `DataPoint_sort` at `kdtrees.c:1226` reorders the list the built tree's node ranges index into, so a later `search()` on the same `KDTree` silently misses points. Silent wrong answer, found during [#74](https://github.com/dbolser/BioPAIthon/pull/74); the fork's rewrite there removes the shared state wholesale rather than fixing this narrowly, so describe the defect, not the patch. |
