...
| # | subject | note |
|---|---|---|
| 1 | `Record.__str__` corrupts a string-valued `wgs` into `A-B-C-D-...` | New in 1.88, and the incomplete half of [#5274](https://github.com/biopython/biopython/issues/5274). [#5275](https://github.com/biopython/biopython/pull/5275) fixed the parsed-list path but left `__init__` at `self.wgs = ""`, so a string assignment now serialises one character at a time. `wgs_scafld` was corrected to `[]` in the same commit. Comment on #5275. **Fixed here**, so the report describes a defect this tree no longer has. |
| 2 | `RestrictionBatch.format` docstring still promises expression evaluation | Documentation only. The 1.88 `eval` removal changed the behaviour and left the docstring saying "If y can be evaluated to a RestrictionType". **Fixed here.** |
