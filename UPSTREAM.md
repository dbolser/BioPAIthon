...
| # | subject | note |
|---|---|---|
| 1 | `Record.__str__` corrupts a string-valued `wgs` into `A-B-C-D-...` | New in 1.88, and the incomplete half of [#5274](https://github.com/biopython/biopython/issues/5274). [#5275](https://github.com/biopython/biopython/pull/5275) fixed the parsed-list path but left `__init__` at `self.wgs = ""`, so a string assignment now serialises one character at a time. `wgs_scafld` was corrected to `[]` in the same commit. Comment on #5275. **Fixed here in [#62](https://github.com/dbolser/BioPAIthon/pull/62)**, so the report describes a defect this tree no longer has. |
| 2 | `RestrictionBatch.format` docstring still promises expression evaluation | Documentation only. The 1.88 `eval` removal changed the behaviour and left the docstring saying "If y can be evaluated to a RestrictionType". **Fixed here in [#62](https://github.com/dbolser/BioPAIthon/pull/62).** |
...
| 18 | `neighbor_simple_search` re-sorts the KD-tree's shared point list | `DataPoint_sort` at `kdtrees.c:1226` reorders the list the built tree's node ranges index into, so a later `search()` on the same `KDTree` silently misses points — measured 623 → 397 hits on 2,000 points. Silent wrong answer. **Fixed here in [#74](https://github.com/dbolser/BioPAIthon/pull/74)**, which sorts a private copy so the tree is immutable after construction; the fork's fix is bundled with a larger rewrite, so describe the defect, not the patch. |
| 19 | `SeqIO.index()` on pretty-printed UniProt XML is silently empty | The `b"<entry "` marker at `_index.py:198` is matched at line start only; an indented `<entry>` never matches, `parse()` reads the same file fine. `Tests/SwissProt/multiligand.xml` in both trees demonstrates. **Fixed here in [#88](https://github.com/dbolser/BioPAIthon/pull/88)** as a side effect of moving id rules onto the parsers — the narrow fix upstream is a `lstrip` or regex allowance. |
| 20 | FOGSAA sizes its matrix in `int` arithmetic | `PyMem_Calloc((nA+1) * (nB+1), ...)` at `_pairwisealigner.c:6075` and the matching `MATRIX()` index macro wrap for two sequences ≳46,341 nt, under-allocate, and corrupt the heap — segfault from pure Python, reproduced in this tree pre-fix. **Fixed here in [#67](https://github.com/dbolser/BioPAIthon/pull/67)** with checked `size_t` sizing. Caller-driven, not file-driven, so the public tracker is the right venue (same reasoning as #5272). |
| 21 | [#3911](https://github.com/biopython/biopython/pull/3911)'s newline check never matches list-valued qualifiers | `"\n" in feature.qualifiers[k]` is element membership on a list, so the list-of-strings shape the motivating SnapGene case produces is untouched by the fix. Comment on #3911 rather than a new issue. Fork adopted the PR and corrected this in [#80](https://github.com/dbolser/BioPAIthon/pull/80). |
| 22 | `System.stochiometry()` has never worked | Three defects in one method, all verbatim from the 2001 import: `[] * n` is `[]`, rows created as the integer `0`, and coefficients written with no row index. Any non-empty system raises `IndexError`; nothing in either tree calls it, which is how it survived 24 years. **Fixed here in [#97](https://github.com/dbolser/BioPAIthon/pull/97)**; found by mypy's `check_untyped_defs` during [#96](https://github.com/dbolser/BioPAIthon/pull/96). |
